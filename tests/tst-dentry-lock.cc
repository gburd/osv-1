...
//
// The inversion: namei() holds vn_lock(dvp) while calling
...
//
// This test does NOT reproduce that deadlock, and cannot on ramfs.  Hitting it
// requires two distinct dentries aliasing one vnode (see the fix commit), i.e.
// two hardlinks to one file, and ramfs_link() is vop_eperm (ramfs_vnops.cc),
// so the precondition cannot be constructed here.  Reproducing it needs a
// filesystem with working link(), plus a rename of one link racing the last
// close() of the other.
//
// What this test does cover: many threads doing open()/close() of different
// names in one directory, with concurrent rename() and unlink() traffic, so it
// exercises dentry_lookup()/dentry_alloc()/dentry_move()/dentry_remove() under
// contention and would catch a crash, a corrupted hash chain, or a leak there.
// Built with DEBUG_VFS it also exercises the vn_lock() order assertion, which
// is the cheap way to catch a reintroduced inversion without needing to race.
// It passes both before and after the fix; treat it as a regression net for
// the dentry cache, not as a reproducer for the inversion.
