 ...
 * across the call.  With only sp installed, the child resumes holding the CHILD
 * THREAD's register values.  x19-x28 are carried across unchanged; the frame
 * pointer x29 is NOT restored (see the comment at the restore site).
 */
                           ...
                           void *resume_ctx, void **out_stack_to_free)
{
    ...
    // The child's resume context: x19-x28 hold plain VALUES (where the compiler
    // parks a caller's live locals across a call) and are carried across
    // unchanged; sp points at the child's copied stack.  The frame pointer x29
    // is deliberately NOT restored, for the reason given in arch/x64/fork.cc:
    // this port relocates the stack, so a frame pointer needs a bias, and the
    // bias is wrong for an outermost frame whose x29 lies outside the copied
    // region (a fork() called straight from main()).
