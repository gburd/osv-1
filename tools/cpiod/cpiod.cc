...
int main(int ac, char** av)
{
    ...
    // upload_manifest.py passes the prefix with a trailing slash ("/zfs/"),
    // so normalize before comparing: without this unmount the dirty ARC data
    // written above is never flushed to the pool vdev (zfs_osv_unmount runs
    // txg_wait_synced + dmu_objset_disown), and the image boots with an empty
    // root dataset.
