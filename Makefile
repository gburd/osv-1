...
# and log any patch that cannot apply -- a single `git apply <all>` is atomic
# per-invocation and silently applies NOTHING if any dependent patch fails its
# pre-flight check, which used to leave the ZFS sources unpatched while the
...
$(out)/core/net_channel.o: CXXFLAGS += -faligned-new
