...
// idle CPU still decays to the floor and halts (measured: adaptive burns ~1/3
// the idle CPU of the fixed 10000), so the high cap costs nothing when idle.
// OSV_IDLE_SPIN_ADAPTIVE=0 pins the window at the cap (the old fixed-count
...
#if CONF_fork
                // A preempted (queued) app thread may be parked on THIS cpu's
                // list (park_timers runs on every app-thread switch-out).
                // Unlink it here, on its owning (source) cpu, before it migrates
                // to `min`, so it is never left linked on a foreign cpu's
                // parked list.
