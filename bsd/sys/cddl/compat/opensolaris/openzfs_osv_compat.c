...
/*
 * taskq_init_ent - initialize a pre-allocated task entry.
 */
void
...
/*
 * taskq_empty_ent - true if the entry is not in a pending dispatch.
 */
int
...
/*
 * taskq_dispatch_ent - dispatch using a pre-allocated entry.
 * On OSv, we fall back to a normal taskq_dispatch (which allocates its
 * own ostask) and record the returned ID in e->tqent_id so that callers
 * can poll/wait on the entry via taskq_empty_ent / taskq_wait_id.
 * This loses the allocation-free property but is functionally correct.
 */
void
    ...
    taskq_ent_t *e)
{
	e->tqent_func = func;
	e->tqent_arg  = arg;
	e->tqent_id   = taskq_dispatch(tq, func, arg, flags);
}
