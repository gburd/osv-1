...
// ---- leak-probe accessors (OSV_LEAK_PROBE) --------------------------------
// Report the sizes of the fork bookkeeping maps so a periodic probe can prove
// empirically whether they stay flat across sustained fork/reap churn (the
// reaper .erase()s each on reap; this measures whether that actually holds).
// fd_tables reports the count of address spaces with tracked inherited fds.
...
{
    { SCOPE_LOCK(g_lock);     if (children)  *children  = g_children.size(); }
    { SCOPE_LOCK(g_pid_lock); if (as_pid)    *as_pid    = g_as_pid.size();
                              if (pid_as)    *pid_as    = g_pid_as.size(); }
    { SCOPE_LOCK(g_fd_lock);  if (fd_tables) *fd_tables = g_inherited_fds.size(); }
    { SCOPE_LOCK(g_sig_lock); if (sigtables) *sigtables = g_child_sigtables.size(); }
}

// Leak-probe (OSV_LEAK_PROBE): total count of live child-opened-fd tracking
// entries across all address spaces + count of owner-released slots kept alive.
// These are the fds a backend opened AFTER fork; on reap the tracking is
// erased but the underlying gfdt slots are NOT closed -- so gfdt occupancy
// (leak_probe_gfdt_count) climbs even while these tracking maps stay flat.
void leak_probe_fd_stats(unsigned long *child_opened_entries,
                         unsigned long *owner_released)
{
    SCOPE_LOCK(g_fd_lock);
    if (child_opened_entries) {
        unsigned long n = 0;
        for (auto &kv : g_child_opened_fds) n += kv.second.size();
        *child_opened_entries = n;
    }
    if (owner_released) *owner_released = g_owner_released_fds.size();
}
