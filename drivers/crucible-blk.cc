...
struct crucible_priv {
    /*
     * MUST be first: multiplex_strategy() reads the real strategy function
     * through *(devop_strategy_t *)dev->private_data, the same convention
     * virtio-blk's blk_priv and the other block drivers use.
     */
...
/*
 * Read/write operations, same as every other block driver.
 *
 * bdev_read/bdev_write take the strategy fast path for any sector-aligned
 * uio and only fall back to the buffer cache for a sub-sector transfer,
 * which a Crucible volume cannot serve anyway (read_sync/write_sync reject
 * anything not a multiple of the volume block size).  So for the aligned
 * I/O ZFS actually issues, these deliver the uio to crucible_strategy()
 * with no buffer cache involvement.
 */
 ...
 *
 * Reached via multiplex_strategy(), which has already added dev->offset (so a
 * partition subdevice addresses correctly) and split the transfer to
 * dev->max_io_size.
 */
                  ...
                  uint32_t block_size, bool read_only, int device_index)
{
    ...
    try {
        ...
        auto* prv = static_cast<struct crucible_priv*>(dev->private_data);
        prv->strategy = crucible_strategy;
