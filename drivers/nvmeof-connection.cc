    ...
    : host_(host), port_(port)
{
    ...
    // Disable Nagle: each synchronous NVMe/TCP command sends one small PDU
    // and blocks for the response, so Nagle + delayed-ACK would add a
    // round-trip of latency per command.  TCP_NODELAY pushes each PDU out
    // immediately.
    int opt = 1;
    setsockopt(fd_, IPPROTO_TCP, TCP_NODELAY, &opt, sizeof(opt));

    // Enable TCP keepalive so the OS detects a dead target connection and
    // unblocks blocked recv() calls rather than hanging indefinitely.
    opt = 1;
    ...
    // Declare the connection dead after 3 unanswered probes.
    opt = 3;
...
{
    /*
     * Serialise concurrent senders.  Multiple OSv block-layer threads may
     * submit commands simultaneously; without this lock their PDU bytes
     * can interleave on the socket and the target resets the connection.
     */
...
{
    /*
     * Holding the lock across BOTH the header and the data sends makes the
     * PDU atomic against concurrent senders -- an H2CData PDU is one
     * logical unit on the wire even though it is physically split between
     * the PDU header and the data bytes themselves.
     */
    std::lock_guard<std::mutex> guard(send_lock_);
