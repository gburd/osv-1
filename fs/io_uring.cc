...
static void set_cq_overflow_flag_locked(struct io_uring_ctx *ctx, bool on)
{
    ...
    if (on)
        __atomic_or_fetch(&sq->flags, IORING_SQ_CQ_OVERFLOW, __ATOMIC_RELEASE);
    else
        __atomic_and_fetch(&sq->flags, ~IORING_SQ_CQ_OVERFLOW, __ATOMIC_RELEASE);
}
                                ...
                                uint32_t *out_cqe_flags)
{
    *out_cqe_flags = 0;
    ...
    case IORING_OP_POLL_ADD: {
         ...
         * indefinite (one-shot readiness), matching Linux POLL_ADD.
         */
        res = -ECANCELED;
    ...
    case IORING_OP_TIMEOUT: {
        ...
        WITH_LOCK(ctx->mtx) {
            uint32_t start_tail = ctx->cq_ring
                ? __atomic_load_n(
                      &static_cast<struct io_uring_cq_ring *>(ctx->cq_ring)->tail,
                      __ATOMIC_ACQUIRE)
                : ctx->cq.tail;
