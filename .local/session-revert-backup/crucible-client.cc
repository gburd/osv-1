...
int UpsairsClient::read_sync(uint64_t offset, uint32_t length, void* buffer)
{
    ...
    // Allocate job ID.  A Read carries no dependencies: ZFS only issues a read
    // after the writes it depends on have already returned (and thus reached
    // quorum), so there is no outstanding write a read must be ordered behind.
...
int UpsairsClient::write_sync(uint64_t offset, uint32_t length, const void* buffer)
{
    ...
    /*
     * A Write's id can become a later Flush's dependency, so it must be
     * durable on EVERY connected replica before we let the baseline advance
     * past it: this upstairs has no live-repair, so a replica that misses a
     * committed-dependency job can never catch up and would wedge every later
     * job that depends on it ("job N waiting on K deps" on all replicas).
     * Require all currently-connected downstairs to ack -- not a fixed 2/3.
     */
    int needed = connected_count_;
    auto req = request_mgr_.create_request(job_id, needed);
    ...
    if (sent_count < needed) {
        request_mgr_.remove_request(job_id);
        kprintf("[Crucible] write job_id=%lu: only %d of %d connected downstairs reachable\n",
                job_id, sent_count, needed);
        return EIO;
    }
...
int UpsairsClient::flush_sync()
{
    ...
    /*
     * A Flush id becomes the dependency baseline (last_flush_id_) for every
     * subsequent Write, so it must be durable on EVERY connected replica
     * before we commit it -- same no-live-repair reasoning as write_sync.
     */
    int needed = connected_count_;
    auto req = request_mgr_.create_request(job_id, needed);
    ...
    if (sent_count < needed) {
        request_mgr_.remove_request(job_id);
        kprintf("[Crucible] flush job_id=%lu: only %d of %d connected downstairs reachable\n",
                job_id, sent_count, needed);
        return EIO;
    }
    ...
    // The barrier was advanced in begin_flush() (allocation order == watermark
    // order); reaching quorum here only confirms durability.
    request_mgr_.remove_request(job_id);
    return 0;
}
...
int UpsairsClient::create_snapshot(uint64_t snapshot_id)
{
    ...
    // The barrier was advanced in begin_flush(); reaching 3/3 only confirms
    // durability of the snapshot point-in-time.
    request_mgr_.remove_request(job_id);
    return 0;
}
...
std::vector<uint64_t> UpsairsClient::begin_write(uint64_t& job_id)
{
    WITH_LOCK(dep_mtx_) {
        ...
        // Record the write as in-flight in the SAME critical section that
        // allocates its id.  The Crucible downstairs tracks completion as a
        // monotonic watermark: a Flush ack collapses its completed set down to
        // the flush id, so any job that lists a dependency below the current
        // flush baseline wedges forever.  The invariant that keeps that safe
        // is that dependencies are assigned in id-allocation order.  If we
        // deferred recording the write until it reached quorum (the old
        // commit_write path), a Flush allocated while this write was still on
        // the wire would not list it as a dependency, would complete, and
        // would advance the downstairs watermark past this write's own
        // flush dependency -- stranding it (observed: writes 363-374 wedged on
        // flush 309 on a stable connection).  Recording at allocation makes
        // every later Flush depend on this write, so the watermark cannot move
        // past a still-needed id.  A write that later fails quorum is removed
        // again by abort_write().
...
{
    WITH_LOCK(dep_mtx_) {
        // The write failed to reach quorum and returned EIO; drop it so a
        // Flush allocated after this point does not depend on an id that was
        // never durably accepted.  (A Flush allocated before the failure may
        // already list it, but that is masked by the EIO the write returns to
        // ZFS, which aborts the txg regardless.)
        auto it = std::find(pending_writes_.begin(), pending_writes_.end(),
                            job_id);
...
std::vector<uint64_t> UpsairsClient::begin_flush(uint64_t& job_id)
{
    WITH_LOCK(dep_mtx_) {
        job_id = job_allocator_.allocate();
        // This flush depends on every write allocated since the previous flush
        // (all of which have smaller ids) plus that previous flush, so the
        // chain of txg barriers stays totally ordered.  Advance the barrier
        // HERE, in the allocation critical section, rather than after quorum:
        // assigning the dependency baseline in id-allocation order is exactly
        // what the downstairs's monotonic completion watermark requires.  A
        // subsequent write reads this flush id as its baseline; the downstairs
        // received the flush (we send to every connected replica) so it can
        // never strand the write.  A flush that fails quorum returns EIO,
        // which is fatal to the ZFS txg anyway, so we do not roll the barrier
        // back.
...
void UpsairsClient::reset_dependency_baseline()
{
    WITH_LOCK(dep_mtx_) {
        /*
         * A reconnected downstairs comes back with an empty `completed` set:
         * this upstairs has no live-repair/replay, so it never re-sends the
         * jobs the downstairs missed while it was gone.  Any later job that
         * lists a pre-reconnect id in its dependency list would therefore
         * block on that downstairs forever ("job N waiting on K deps"), and
         * if the missed id was a flush that all replicas later recycled past,
         * the job wedges at 0/3 -- which surfaces in ZFS as a hung
         * txg_wait_synced and, downstream, the btree VERIFY3P panic.
         *
         * Clearing the baseline only ever removes ordering constraints: every
         * flush we drop here already reached quorum and is durable on the
         * replicas that stayed up, and is by definition absent on the one that
         * just reconnected.  The next Write/Flush starts a fresh dependency
         * chain that every currently-connected downstairs can satisfy.
         */
        pending_writes_.clear();
        last_flush_id_ = 0;
    }
                ...
                if (connections_[i] && connections_[i]->is_connected()) {
                    ...
                    if (FD_ISSET(fd, &readfds)) {
                        try {
                            ...
                            // The dropped downstairs loses every in-flight job
                            // and will return with an empty completed set (no
                            // live-repair).  Drop the dependency baseline now,
                            // before any further Write/Flush is built, so no
                            // later job lists an id this downstairs will never
                            // hold and then wedges on it forever.
                            reset_dependency_baseline();
