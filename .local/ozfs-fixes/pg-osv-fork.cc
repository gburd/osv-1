...
sched::thread *fork_thread(void *caller_ret, void *caller_sp, void **out_stack_to_free)
{
    ...
    debug_early_u64("FORK si.begin=", (u64)si.begin);
    debug_early_u64("FORK stack_base=", (u64)stack_base);
    debug_early_u64("FORK caller_sp=", (u64)sp);
    debug_early_u64("FORK live_bytes=", (u64)(stack_base - sp));
    ...
    // Capture the parent thread's ACTUAL fsbase (glibc TLS base).  The app_tcb
    // mechanism only records a base for threads whose TLS was installed via
    // arch_prctl(SET_FS); the app's main thread (and thus a postmaster forking
    // from it) may have app_tcb==0 while running on a real non-zero fsbase.
    // The child must resume on the SAME TLS the caller used, so read the live
    // fsbase directly and restore it in the child.
